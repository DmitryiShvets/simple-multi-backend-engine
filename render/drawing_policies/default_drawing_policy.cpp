...
void DefaultDrawingPolicy::render(CommandList& cmd, const DrawingData& data) {
    ...
    assert(data.hasPushConstants() && "DrawingData: push_constants (modelMatrix) is required");
    ...
    auto it = data.push_constants.find("modelMatrix");
    assert(it != data.push_constants.end() && "DrawingData: modelMatrix push constant is required");
