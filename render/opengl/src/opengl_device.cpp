...
OpenGLDevice::OpenGLDevice(OpenglResourceManager &resource_manager, PipelineConfigRegistry& pl_registry)
...
RID OpenGLDevice::createDescriptorSetLayout(
  ...
  for (const auto& binding : desc.bindings) {
    layout->addBinding(binding.binding,
...
RID OpenGLDevice::createDescriptorSet(RID layout_rid, const std::vector<RID>& buffer_rids) {
  // Get layout (for validation, optional)
  auto layout = m_resource_manager.get_ptr<OpenGLDescriptorSetLayout>(layout_rid);
  if (!layout) {
...
RID OpenGLDevice::createMaterial(const std::string &material_name, const UniformSet& material_uniforms) {
    ...
    // Get the binding from the pipeline config (should be 1 for OpenGL MaterialUBO)
...
void OpenGLDevice::updateBufferRaw(RID rid, size_t offset, size_t size, const void *data) {
