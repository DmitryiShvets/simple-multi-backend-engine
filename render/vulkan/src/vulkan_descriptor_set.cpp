...
DescriptorSetLayout::DescriptorSetLayout(
  ...
  if (vkCreateDescriptorSetLayout(lveDevice.getDeviceHandle(),
...
DescriptorSetLayout::~DescriptorSetLayout() {
  vkDestroyDescriptorSetLayout(lveDevice.getDeviceHandle(), descriptorSetLayout,
                               nullptr);
}
...
std::unique_ptr<DescriptorPool> DescriptorPool::Builder::build() const {
  return std::make_unique<DescriptorPool>(lveDevice, maxSets, poolFlags,
                                          poolSizes);
}
...
DescriptorPool::DescriptorPool(
    VulkanDevice &lveDevice, uint32_t maxSets,
  ...
  if (vkCreateDescriptorPool(lveDevice.getDeviceHandle(), &descriptorPoolInfo,
...
DescriptorPool::~DescriptorPool() {
  vkDestroyDescriptorPool(lveDevice.getDeviceHandle(), descriptorPool, nullptr);
}
...
bool DescriptorPool::allocateDescriptor(
  ...
  if (vkAllocateDescriptorSets(lveDevice.getDeviceHandle(), &allocInfo,
...
void DescriptorPool::freeDescriptors(
  ...
  vkFreeDescriptorSets(lveDevice.getDeviceHandle(), descriptorPool,
                       static_cast<uint32_t>(descriptors.size()),
                       descriptors.data());
}

void DescriptorPool::resetPool() {
  vkResetDescriptorPool(lveDevice.getDeviceHandle(), descriptorPool, 0);
}
...
DescriptorWriter::writeImage(uint32_t binding,
  ...
  // assert(bindingDescription.descriptorCount == 1 && "Binding single
  // descriptor info, but binding expects multiple");
...
void DescriptorWriter::overwrite(VkDescriptorSet &set) {
  ...
  vkUpdateDescriptorSets(pool.lveDevice.getDeviceHandle(), writes.size(),
