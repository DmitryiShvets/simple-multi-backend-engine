...
void VulkanDevice::initialize(const IVulkanSurfaceCreator &creator) {
...
void VulkanDevice::createSurface(const IVulkanSurfaceCreator &creator) {
  auto _surface = creator.createWindowSurface(*m_instance);
  if (!_surface) {
...
bool VulkanDevice::isDeviceSuitable(const vk::raii::PhysicalDevice &device) {
  ...
  isSuitable =
      isSuitable && findQueueFamilies(device, *m_surface).isComplete();
