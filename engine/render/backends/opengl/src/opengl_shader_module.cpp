...
OpenGLShaderModule::OpenGLShaderModule(const GLenum type,
  ...
  m_shader_module = createShader(code.data(),  static_cast<GLint>(code.size()), type);
}
OpenGLShaderModule::OpenGLShaderModule(const GLenum type,
...
GLuint OpenGLShaderModule::createShader(const char *source, const GLint length,
                                        const GLenum type) {
  GLuint shader_module = glCreateShader(type);
  glShaderSource(shader_module, 1, &source, &length);
