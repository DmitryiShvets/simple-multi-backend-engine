...
const UniformLayout::Variable *
UniformLayout::getVariable(const std::string &name) const {
...
std::vector<uint8_t> UniformLayout::pack(const UniformSet &uniform_set) const {
  if (!m_computed) {
    throw std::runtime_error(
        "UniformLayout::computeLayout() must be called before pack()");
  }
...
void UniformLayout::packTo(const UniformSet &uniform_set,
                           uint8_t *buffer) const {
  if (!m_computed) {
    throw std::runtime_error(
        "UniformLayout::computeLayout() must be called before packTo()");
  }
  ...
  for (const auto &var : m_variables) {
    ...
    if (!value) {
      Logger::warning_log("Wrong uniform layaut: uniform value [" + var.name +
                        "] is not found");
      continue;
    }
    // Type validation
    if (value->getType() != var.type) {
      Logger::warning_log("Wrong uniform layaut: uniform value [" +
                        var.name + "] is fullfil layaut");
      continue;
    }
    // Copy data
...
UniformLayout
UniformLayout::createStd140(std::initializer_list<Variable> vars) {
