...
bool OpenGLGpuContextCreator::createContext(void *window) const {
  ...
  if (version == 0) {
    Logger::error_log("Не удалось инициализровать OpenGL контекст!");
    return false;
  }
  Logger::info_log("Initialized OpenGL version " + std::to_string(GLAD_VERSION_MAJOR(version)) +
...
bool VulkanGpuContextCreator::createContext(void *window) const {
  // Для Vulkan создание контекста (VkSurface) происходит на более позднем
  // этапе, через другую стратегию (SurfaceCreator), как описано в
  // архитектурных документах. Здесь действий не требуется.
  return true;
}
